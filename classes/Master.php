...
Class Master extends DBConnection {
	...
	function save_shop_type(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Danh mục cửa hàng đã tồn tại.";
		}else{
			if(empty($id)){
			...
			if($save){
				$resp['status'] = 'success';
				if(empty($id))
				$resp['msg'] = " Danh mục cửa hàng mới đã lưu thành công.";
				else
				$resp['msg'] = " Cập nhật danh mục cửa hàng thành công.";
			}else{
				$resp['status'] = 'failed';
	...
	function delete_shop_type(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Danh mục cửa hàng xóa thành công.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_category(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Danh mục sản phẩm đã tồn tại.";
		}else{
			if(empty($id)){
			...
			if($save){
				$resp['status'] = 'success';
				if(empty($id))
				$resp['msg'] = " Danh mục sản phẩm mới được lưu thành công.";
				else
				$resp['msg'] = " Danh mục sản phẩm được cập nhật thành công.";
			}else{
				$resp['status'] = 'failed';
				$resp['err'] = $this->conn->error."[{$sql}]";
			}
	...
	function save_product(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = ' Tên sản phẩm đã tồn tại.';
		}else{
			if(empty($id)){
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = " Sản phẩm mới được lưu thành công.";
				else
					$resp['msg'] = " Sản phẩm được cập nhật thành công.";
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(!in_array($type,$allowed)){
						$resp['msg']=" Ảnh không tải được do file không hợp lệ.";
					}else{
						...
						$resp['msg']=" Ảnh tải lên lỗi.Vui lòng thử lại";
						}
					}
				...
				$resp['status'] = 'failed';
				if(empty($id))
					$resp['msg'] = " Lưu thông tin sản phẩm thất bại.";
				else
					$resp['msg'] = " Sản phẩm cập nhật thất bại.";
				$resp['err'] = $this->conn->error."[{$sql}]";
			}
		}
	...
	function delete_product(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Sản phẩm đã xóa thành công.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function add_to_cart(){
		...
		if($save){
			$resp['status'] = 'success';
			$resp['msg'] = " Sản phẩm được thêm vào giỏ hàng.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " Sản phẩm bị lỗi khi thêm vào giỏ hàng.";
	...
	function update_cart_qty(){
		...
		if($update_cart){
			$resp['status'] = 'success';
			$resp['msg'] = ' Số lượng sản phẩm cập nhật thành công';
		}else{
			$resp['status'] = 'success';
			$resp['msg'] = ' Số lượng sản phẩm cập nhật thất bại';
			$resp['error'] = $this->conn->error;
		}
	...
	function cancel_order(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Đơn hàng đã hủy thành công.";
		}else{
			$resp['status'] = 'success';
			$resp['error'] = $this->conn->error;
		}
	...
	function update_status(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Trạng thái đơn hàng đã cập nhật thành công.";
		}else{
			$resp['status'] = 'success';
			$resp['msg'] = " Trạng thái đơn hàng đã cập nhật thất bại.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] == 'success')
		$this->settings->set_flashdata('success',$resp['msg']);
