...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(!in_array($type,$allowed)){
						$resp['msg']=" Lỗi tải ảnh do file ảnh không hợp lệ.";
					}else{
						$new_height = 200; 
						$new_width = 200; 
						...
						$resp['msg']=" Lỗi tải ảnh. Vui lòng thử lại";
						}
					}
			...
			if($qry){
				$this->settings->set_flashdata('success','Thông tin được cập nhật thành công.');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					$allowed = array('image/png','image/jpeg');
					if(!in_array($type,$allowed)){
						$resp['msg']=" Tải ảnh lỗi do ảnh không hợp lệ.";
					}else{
						$new_height = 200; 
						$new_width = 200; 
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','Đã xóa thành công người dùng.');
	...
	public function save_vendor(){
		...
		if(isset($oldpassword) && !empty($id)){
			...
			if(md5($oldpassword) != $current_pass){
				$resp['status'] = 'failed';
				$resp['msg'] = ' Mật khẩu hiện tại chưa chính xác';
				return json_encode($resp);
				exit;
			}
		}
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Tên đăng nhập đã tồn tại";
		}else{
			if(empty($id)){
			...
			if($save){
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'vendor/register.php') > -1){
						$resp['msg'] = " Tài khoản của bạn đã đăng ký thành công.";
					}else{
						$resp['msg'] = " Tài khoản nhà cung cấp đã đăng ký thành công.";
					}
				}else{
					if($this->settings->userdata('login_type') == 2){
						$resp['msg'] = " Tài khoản của bạn đã được cập nhật thành công.";
					}else{
						$resp['msg'] = " Tài khoản nhà cung cấp đã được cập nhật thành công.";
					}	
				}
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					$allowed = array('image/png','image/jpeg');
					if(!in_array($type,$allowed)){
						$resp['msg'].=" Tải ảnh không thành công do file ảnh không hợp lệ.";
					}else{
						$new_height = 200; 
						$new_width = 200; 
				...
				$resp['status'] = 'failed';
				$resp['msg'] = " Đã có lỗi xảy ra khi lưu thông tin.";
	...
	public function delete_vendor(){
		...
		if($qry){
			$this->settings->set_flashdata('success',' Xóa thông tin nhà cung cấp thành công.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Xảy ra lỗi khi xóa dữ liệu.';
			$resp['error'] = $this->conn->error;
		}
		return json_encode($resp);
	}
			...
			if($save){
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'client/register.php') > -1){
						...
						$resp['msg'] = " Tài khoản khách hàng đã đăng ký thành công.";
					}
				}else{
					if($this->settings->userdata('login_type') == 3){
						$resp['msg'] = " Thông tin của bạn đã được cập nhật thành công.";
					}else{
						$resp['msg'] = " Thông tin khách hàng được cập nhật thành công";
					}	
				}
	...
	public function delete_client(){
		...
		if($qry){
			$this->settings->set_flashdata('success',' Xóa khách hàng thành công.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = 'Đã xảy ra lỗi khi xóa.';
			$resp['error'] = $this->conn->error;
		}
		return json_encode($resp);
	}
